...
class DAOProspect{
    ...
    public function incluirProspect($nome, $email, $celular, $facebook, $whatsapp){
        ...
        $sqlInsert = $conexaoDB->prepare("insert into prospect (nome, email, celular, facebook, whatsapp) values (?,?,?,?,?)");
     ...
     * @param Prospect $prospect Objeto Prospect com os dados atualizados
     * @return TRUE|Exception
     */
    public function atualizarProspect($prospect){
        ...
        $sqlUpdate = $conexaoDB->prepare("update prospect set nome = ?, email = ?, celular = ?, facebook = ?, whatsapp = ? where cod_prospect = ?");
     ...
     * @param int $codProspect Código do Prospect que deve ser excluído
     * @return TRUE|Exception
     */
    public function excluirProspect($codProspect){
        ...
        $sqlDelete = $conexaoDB->prepare("delete from prospect where cod_prospect = ?");
    ...
    public function buscarProspects($email=null){
        ...
        if($email === null){
            $sqlBusca = $conexaoDB->prepare("select cod_prospect, nome, email, celular, facebook, whatsapp from prospect");
            $sqlBusca->execute();
        }else{
            $sqlBusca = $conexaoDB->prepare("select cod_prospect, nome, email, celular, facebook, whatsapp from prospect where email = ?");
        ...
        if($resultado->num_rows !== 0){
            while($linha = $resultado->fetch_assoc()){
                ...
                $prospect->addProspect($linha['cod_prospect'], $linha['nome'], $linha['email'], $linha['celular'], $linha['facebook'], $linha['whatsapp']);
                $prospects[] = $prospect;
            }
        }
